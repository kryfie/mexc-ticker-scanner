...
def main() -> None:
    ...
    debug_rows: List[Dict[str, Any]] = []
    now = int(time.time())
    requested_start = now - args.days * 24 * 60 * 60

    for symbol_index, symbol in enumerate(symbols, start=1):
        max_leverage = meta_map.get(symbol).max_leverage if meta_map.get(symbol) else None
        print(f"[{symbol_index}/{len(symbols)}] {symbol} | max leverage={max_leverage}", flush=True)

        for interval in args.intervals:
            print(f"  downloading {interval} once...", flush=True)
            warmup_start = requested_start - 500 * INTERVAL_SECONDS[interval]
            prepared: Optional[pd.DataFrame] = None

            for attempt in range(1, 4):
                try:
                    raw = fetch_klines(symbol, interval, warmup_start, now)
                    prepared = prepare_data(raw)
                    break
                except Exception as exc:
                    print(f"  ERROR download {interval} attempt {attempt}/3: {exc}", flush=True)
                    if attempt < 3:
                        time.sleep(3 * attempt)

            if prepared is None:
                print(f"  FAILED {symbol} {interval}", flush=True)
                continue

            for tp_r in args.tp_r_values:
                for max_sl1_roi in args.max_sl1_roi_values:
                    cfg = ScanConfig(
                        interval=interval,
                        days=args.days,
                        tp_r=tp_r,
                    ...
                    row = asdict(result)
                    row["eligible_min_trades"] = result.closed_trades >= args.min_closed_trades
                    row["is_profitable"] = result.ending_capital_usdt > result.starting_capital_usdt
                    all_rows.append(row)

                    # Debug only for eligible configurations to keep artifact size manageable.
